...
const STRICT_LARGE_NO_PIPE_PLUGIN_FORMAT_TIMEOUT_MS = 2_500;
...
function expectInvalidMarkdownTableStyleMessage(
  ...
  expect(
    message.includes('markdownTableStyle') ||
      message.includes('markdown-table-style'),
  ).toBe(true);
  expect(message).toContain(`received "${invalidStyle}"`);
